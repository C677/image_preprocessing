...
def show_nodules(ct_scan, nodules, Origin, Spacing, radius=20, pad=2, max_show_num=4): # radius is half of the square side length, pad is the width of the edge, max_show_num maximum number of impressions
    ...
    for idx in range(nodules.shape[0]): # lable is an array of nx4 dimensions, n is the number of lung nodules, 4 is x, y, z, and diameter
        if idx < max_show_num:
            ...
            data[max(0, y - radius):min(data.shape[0], y + radius),
                min(data.shape[1], x + radius):min(data.shape[1], x + radius + pad)] = 3000 #vertical line
    ...
    for sub_n in range(1):
        ...
        for i in image_paths:
            ...
            if series_uid in annotations.keys():
                ...
                show_nodules(numpyImage,nodule_coords,OR,SP)